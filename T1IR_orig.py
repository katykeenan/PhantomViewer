...
def initializeT1IR (nroi=None,TI=None, data=None, TR=10000, roi = None, useROIs = False):
...
def T1IR(params, TI, data):
    ...
    # or to account for non-infinite TR: model = Si*(1+(B-1)*np.exp(-TR/T1)-B*np.exp(-TI/T1))
    return (model - data)

def fitT1IR(params, TI, data):
    """fits signal vs TI data to T1IR model"""
